...
export default function AdminHomePage() {
    ...
    const setupIssueModal = async () => {
        ...
        //         .then(res => res.data.map((issue: any) => issues.push([issue._id, issue.status, issue.title])));
        //         // .then(res => res.data.map((issue: any) => console.log(issue)));
        //         // .then(info => console.log(info));
        //         // .then(info => setIssues(info)
        //         // );
        // } catch (error) {
        //     console.log("HHERHEHRHEHRHE");
        ...
        // var index = issues.findIndex(([id, status, title]) => status === "Current");
        // // var [id, status, title] = issues[index][0];
        // // issues.at
        // try {
        //     setCurrent(issues[index][0]);
        // } catch (error) {
        //     console.error("Error accessing issues: ", error);
        // }
        console.log(current);
        setShowModal(true);
    }
    ...
    function IssueModal() {
        ...
        const [tempIssueID, setTempIssueID] = React.useState(1);
        ...
        async function saveNewIssue() {
            ...
            //     console.log(error);
            // }
            // const ID = tempIssueID.toString()
            issues.push([tempIssueID.toString(), "Next", inputValue]);
            setTempIssueID(tempIssueID + 1);
            addedIDS.push(tempIssueID.toString());
        ...
        async function closeModal() {
            ...
            if (addedIDS.length > 0) {            
                addedIDS.map((newID: any) => {
                    var index = issues.findIndex(([id, status, title]) => id === newID);
                    var [id, status, title] = issues[index];
                    try {
                        // add submission to database
                    ...
                    if (newID == current) {
                        try {
                            fetch("../api/issues/get", { method: "GET" })
                                .then(response => response.json())
                                // .then(res => console.log(res));
                                .then(res => res.data.map((issue: any) => {
                                    if (issue.title == title) {
                                        setCurrent(issue._id);
                                }}));
                                // .then(res => res.data.map((issue: any) => console.log(issue)));
                                // .then(info => {
                                //     issues[index] = (info);
            ...
            // res.data.map((issue: any) => [issue._id, issue.status, issue.title]))
            ...
            if (addedisCurr) {
                ...
                var index = issues.findIndex(([id, status, title]) => id === current);
                var changed_index = issues.findIndex(([id, status, title]) => status === "Current");
                console.log(index);
                                                        ...
                                                        {(status === "Current") ? (
                                                            <div>
                                                                <div className="flex items-center mb-4">
                                                                    <input id="default-radio-1" type="radio" value="" defaultChecked={status == "Current"} name="default-radio" className="p-[0px] m-[0px] text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                                                                </div>
                                                            ...
                                                            <div>
                                                                <div className="flex items-center mb-4">
                                                                    <input id="default-radio-1" type="radio" value="" onChange={() => setCurrent(id)} name="default-radio" className="text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"/>
                                                                </div>
